using DataCenter.Network.HttpWeb.Attributes;
using DataCenter.Network.HttpWeb.Json;
...
namespace DataCenter.Network.HttpWeb
{
    public static class Router
    {
        ...
        public static ExecutionResult Execute(RequestContext requestContext, string jsonData, string target)
        {
            ...
            {
                Application.Error("Declined request from ", requestContext.Address);
                Application.Error("Invalid access token = {0}", requestContext.AccessToken);
            ...
            {
                Application.Error("Declined request from ", requestContext.Address);
                Application.Error("Invalid UnitId value = {0}", requestContext.UnitId);
                return CreateExecutionResult(HttpStatusCode.BadRequest);
            }
            ...
            catch (TargetInvocationException exception)
            {
                ...
                Application.Crash(originalException.Message, originalException.StackTrace.Split("\n\r"));
                return CreateExecutionResult(HttpStatusCode.InternalServerError);
            }
            catch (Exception exception)
            {
                Application.Crash(exception.Message, exception.StackTrace.Split("\n\r"));
                return CreateExecutionResult(HttpStatusCode.InternalServerError);
            }
        }
